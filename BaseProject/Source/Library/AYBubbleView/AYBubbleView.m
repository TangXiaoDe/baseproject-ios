...
{
    self.unReadLabel.text = text;
    self.hideLabel.text = text;
    if(text.length == 1) {
        self.frontView.bounds = CGRectMake(0, 0, 18, 18);
        self.backView.bounds = CGRectMake(0, 0, 18, 18);
    } else if(text.length == 2) {
        self.frontView.bounds = CGRectMake(0, 0, 25, 18);
        self.backView.bounds = CGRectMake(0, 0, 25, 18);
    } else if(text.length == 3) {
        self.frontView.bounds = CGRectMake(0, 0, 35, 18);
        self.backView.bounds = CGRectMake(0, 0, 35, 18);
    }
    self.frame = self.frontView.frame;
}
...
- (void)config {
    ...
    self.frontView.layer.cornerRadius = self.frontView.bounds.size.height * .5;
    self.backView.layer.cornerRadius = self.backView.bounds.size.height * .5;
    self.frontContainerView = [[UIView alloc] initWithFrame:self.frame];
    CGRect frame = self.frontView.frame;
    frame.size = CGSizeMake(self.frontView.frame.size.width + 30, self.frontView.frame.size.height + 30);
    self.frontContainerView.frame = frame;
    self.frontContainerView.center = self.frontView.center;
    self.backView.hidden = YES;
    [self.superview addSubview:self.frontView];
    [self.superview addSubview:self.backView];
    [self.superview addSubview:self.frontContainerView];
    UIPanGestureRecognizer *panGestureRecongnizer = [[UIPanGestureRecognizer alloc] initWithTarget:self action:@selector(panAction:)];
    [self.frontContainerView addGestureRecognizer:panGestureRecongnizer];
//    [self addBubbleAnimation];
    ...
    self.hideLabel = [[UILabel alloc] initWithFrame:self.backView.bounds];
    self.hideLabel.textAlignment = NSTextAlignmentCenter;
    self.hideLabel.textColor = [UIColor clearColor];
    self.hideLabel.font = [UIFont systemFontOfSize:11 weight:0.05];
    [self.backView addSubview:self.hideLabel];
    
    [self.unReadLabel mas_makeConstraints:^(MASConstraintMaker *make) {
        make.top.equalTo(self.frontView).offset(2);
        make.bottom.equalTo(self.frontView).offset(-2);
        make.left.equalTo(self.frontView).offset(5);
        make.right.equalTo(self.frontView).offset(-5);
    }];
    ...
    if (pan.state == UIGestureRecognizerStateBegan) {
        ...
        if (radius_B >= radius_A / 10) {
            ...
            [UIView animateWithDuration:.5 delay:0 usingSpringWithDamping:.4 initialSpringVelocity:.8 options:UIViewAnimationOptionCurveEaseInOut animations:^{
                ...
                [self setText:self.unReadLabel.text];
